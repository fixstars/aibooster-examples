...
def evaluate_orig(model, criterion, postprocessors, data_loader, base_ds, device, output_dir):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        samples = samples.to(device)
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
        samples = {"tensor": samples.tensors.to("cuda"), "mask": samples.mask.to("cuda")}
        outputs = model(**samples)

        keys = ["pred_logits", "pred_boxes"]
        if len(outputs) == 3:
            keys.append("aux_outputs")
        outputs = {key: value for key, value in zip(keys, outputs)}
        loss_dict = criterion(outputs, targets)
        weight_dict = criterion.weight_dict
        ...
        pad_sizes = torch.stack([t["pad_size"] for t in targets], dim=0)
        results = postprocessors['bbox'](outputs, orig_target_sizes, pad_sizes)
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, trt_engine_dir, trt_config, skip_convert):
    ...
    class Evaluator:
        ...
        def update(self, ret):
            ...
            if len(ret) == 3:
                keys.append("aux_outputs")
            ret = {key: value.to("cpu") for key, value in zip(keys, ret)}
            targets = [self.dataset[i][1] for i in range(self.count, self.count + self.batch_size)]
            orig_target_sizes = torch.stack([t["orig_size"] for t in targets], dim=0)
            pad_sizes = torch.stack([t["pad_size"] for t in targets], dim=0)
            results = postprocessors['bbox'](ret, orig_target_sizes, pad_sizes)
            if 'segm' in postprocessors.keys():
                target_sizes = torch.stack([t["size"] for t in targets], dim=0)
    ...
    def post_process(samples):
        return {"tensor": samples[0].tensors.to("cuda"), "mask": samples[0].mask.to("cuda")}
